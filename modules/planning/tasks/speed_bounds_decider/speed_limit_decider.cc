  ...
  for (uint32_t i = 0; i < discretized_path.size(); ++i) {
    ...
    for (const auto* ptr_obstacle : obstacles.Items()) {
      ...
      if (is_close_on_left || is_close_on_right) {
        ...
        if (ptr_obstacle->Perception().theta() > 0.0168 && ptr_obstacle->Perception().theta() < 0.0169) {
            speed_limit_from_nearby_obstacles = 2.5;
        } else {
    ...
    if (obstacles.Items().size() > 20) curr_speed_limit =8.0;
