...
void OpenSpaceRoiDecider::SetParkAndGoEndPose(Frame *const frame) {
  ...
  double target_x = reference_point.x()-50;
  double target_y = reference_point.y();
  double target_theta = reference_point.heading();

  
  AINFO << "targetX: " << std::fixed << std::setprecision(3) << target_x;
  AINFO << "targetY: " << std::fixed << std::setprecision(3) << target_y;
...
bool OpenSpaceRoiDecider::GetParkingBoundary(
  ...
  auto right_down = parking_info.corner_points[2];
  auto right_top = parking_info.corner_points[1];
  AINFO << "parking_spot_corners:" << std::endl
        << "left_top: (" << std::fixed << std::setprecision(3) << left_top.x()
        << ", " << std::fixed << std::setprecision(3) << left_top.y() << ")" << std::endl
        << "left_down: (" << std::fixed << std::setprecision(3) << left_down.x()
        << ", " << std::fixed << std::setprecision(3) << left_down.y() << ")" << std::endl
        << "right_down: (" << std::fixed << std::setprecision(3) << right_down.x()
        << ", " << std::fixed << std::setprecision(3) << right_down.y() << ")" << std::endl
        << "right_top: (" << std::fixed << std::setprecision(3) << right_top.x()
        << ", " << std::fixed << std::setprecision(3) << right_top.y() << ")";
  ...
  if (average_l < 0) {
    ...
    AINFO << "average_l is less than 0 in OpenSpaceROI";
    ...
    right_top.set_x(right_top.x() + 1.0);
    right_down.set_x(right_down.x() + 1.0);
    left_top.set_x(left_top.x() - 3.5);
    left_down.set_x(left_down.x() - 3.5);
    ...
    // lane boundary and assume that the lane half width is average_l
    AINFO << "average_l is greater than 0 in OpenSpaceROI";
  ...
  // START: 自定义障碍物代码添加区域  
  // const double offset_1 = 3.5;
  // const double offset_2 = 14.0;
  // auto left_top_1 = parking_info.corner_points[0];
  // auto left_down_1 = parking_info.corner_points[3];
  // auto right_down_1 = parking_info.corner_points[2];
  // auto right_top_1 = parking_info.corner_points[1];

  // // std::vector<common::math::Vec2d> obstacle_points_1 = {
  // //     common::math::Vec2d(left_top_1.x() + offset_1, left_top_1.y()-0.5),
  // //     common::math::Vec2d(right_top_1.x() + offset_1, right_top_1.y()-0.5),
  // //     common::math::Vec2d(right_down_1.x() + offset_1, right_down_1.y() + 0.5),
  // //     common::math::Vec2d(left_down_1.x() + offset_1, left_down_1.y() + 0.5)
  // // };

  // std::vector<common::math::Vec2d> obstacle_points_2 = {
  //     common::math::Vec2d(left_top_1.x() + offset_2, left_top_1.y()),
  //     common::math::Vec2d(right_top_1.x() + offset_2, right_top_1.y()),
  //     common::math::Vec2d(right_down_1.x() + offset_2, right_down_1.y()),
  //     common::math::Vec2d(left_down_1.x() + offset_2, left_down_1.y())
  // };

  
  // std::vector<std::vector<common::math::Vec2d>> all_obstacles = {
  //     // obstacle_points_1, 
  //     obstacle_points_2
  // };

  // // 遍历所有障碍物
  // for (auto& obstacle_points : all_obstacles) {
  //     // 对每个障碍物的所有点进行坐标转换
  ...
  //     // 将转换后的点分解为线段，并添加到 ROI 边界列表中
  //     for (size_t i = 0; i < obstacle_points.size(); ++i) {
  //         std::vector<common::math::Vec2d> obstacle_segment;
  //         obstacle_segment.push_back(obstacle_points[i]);
  //         if (i == obstacle_points.size() - 1) {
  //             obstacle_segment.push_back(obstacle_points[0]);
  //         } else {
  //             obstacle_segment.push_back(obstacle_points[i + 1]);
  //         }
  //         roi_parking_boundary->push_back(obstacle_segment);
  //     }
  // }
  // END: 自定义障碍物代码添加区域
  ...
  // 增加 ROI 区域的示例
  const double expansion_distance = 2.0;
  ROI_xy_boundary[0] -= expansion_distance; // xmin
  ROI_xy_boundary[1] += expansion_distance; // xmax
  ROI_xy_boundary[2] -= expansion_distance; // ymin
  ROI_xy_boundary[3] += expansion_distance; // ymax
