...
double LaneBorrowPath::AddAdditionalBound() {
  const auto& obstacles = reference_line_info_->path_decision()->obstacles().Items();
  const auto& obstacles_count = reference_line_info_->path_decision()->obstacles().Items().size();
  int type5_count = 0;
  ...
  if (type5_count > 10 ) {
    return 7.0;//换为7施工区域的 1 2 能通过，6只有1能通过
  }
  if(obstacles_count >= 25){
    return 5.0;
  }
  for (const auto* obstacle : obstacles) {
    if (obstacle->Perception().theta() > 0.0168 && obstacle->Perception().theta() < 0.0169) {
      return 1.0;
    }
  }
  if(obstacles_count >= 8 && obstacles_count <= 12){
    return 0.0;
  }else if(obstacles_count > 12 && obstacles_count <= 16){
    return 4.0;
  }
...
bool LaneBorrowPath::IsNecessaryToBorrowLane() {
  auto* mutable_path_decider_status = injector_->planning_context()
                                          ...
                                          ->mutable_path_decider();
  const auto& obstacles = reference_line_info_->path_decision()->obstacles().Items();
  AINFO << "当前障碍物数量: " << obstacles.size();
  for (const auto* obstacle : obstacles) {
    AINFO << "障碍物ID: " << obstacle->Id()
          << "  类型: " << obstacle->Perception().type()
          << "  theta角度: " << obstacle->Perception().theta();
  }

  bool find_target_obstacle = false;

  for (const auto* obstacle : obstacles) {
    if (obstacle->Perception().theta() > 0.0168 && obstacle->Perception().theta() < 0.0169) {
      find_target_obstacle = true;
      AINFO << "目标障碍物满足条件" ;
      
      mutable_path_decider_status->set_front_static_obstacle_id(obstacle->Id());
      mutable_path_decider_status->set_front_static_obstacle_cycle_counter(
          config_.long_term_blocking_obstacle_cycle_threshold() + 1);
      mutable_path_decider_status->set_is_in_path_lane_borrow_scenario(true);
      break;
    }
  }
  if (find_target_obstacle) {
