  ...
  *narrowest_width =
  ...
  for (size_t i = 1; i < path_boundary->size(); ++i) {
    ...
    for (size_t j = 0; j < sl_polygon->size(); j++) {
      ...
      const auto& crosswalk_overlaps = reference_line_info.reference_line().map_path().crosswalk_overlaps();
      int crosswalk_count = crosswalk_overlaps.size();
      
      if (crosswalk_count == 2) {
        adc_obs_edge_buffer = adc_half_width;
      } else if (crosswalk_count == 1) {
        adc_obs_edge_buffer = adc_half_width + 1.0;
      }
        ...
        if (sl_polygon->at(j).is_passable()[RIGHT_INDEX]) {
          if (sl_polygon->at(j).is_passable()[LEFT_INDEX]) {
            if (std::fabs(obs_l - mid_l) < 0.4 &&
              ...
              std::unordered_map<std::string, double> obstacle_x_map;
              std::string current_obstacle_id = sl_polygon->at(j).id();
              double current_obstacle_x = 0.0;
              ...
              for (const auto* obstacle : obstacles) {
                  ...
                  if (obstacle->Perception().type() == 5) {
                      ++type5_count;
                      double x = obstacle->PerceptionBoundingBox().center().x();
                      double y = obstacle->PerceptionBoundingBox().center().y();
                      if (x > type5_max_x) {
                          type5_max_x = x;
                          type5_y = y;
                      }
              ...
              double nudge_threshold = 0.0;
              if (obstacles.size() > 30) {
                  double region_x_min = type5_max_x - 47.4;
                  double region_x_max = type5_max_x + 111.2;
                  double region_y_min = type5_y - 10.1;
                  double region_y_max = type5_y + 1.8;
                  
                  for (const auto* obstacle : obstacles) {
                      if (!obstacle) continue;
                      if (obstacle->Id() == "DEST") continue;
                      
                      const auto& center = obstacle->PerceptionBoundingBox().center();
                      double x = center.x();
                      double y = center.y();
                      ...
                      obstacle_x_coords.push_back(x);
                      obstacle_x_map[obstacle->Id()] = x;
                      if (obstacle->Id() == current_obstacle_id) {
                          current_obstacle_x = x;
                      }
                  }
                  
                  
                  if (!obstacle_x_coords.empty()) {
                      std::sort(obstacle_x_coords.begin(), obstacle_x_coords.end());
                      double leftmost_x = obstacle_x_coords.front();
                      double distance_to_leftmost = current_obstacle_x - leftmost_x;
                      
                      if ((distance_to_leftmost < 37.0 && distance_to_leftmost > 0.0) ||
                          distance_to_leftmost > 150) {
                          nudge_threshold = -2.0;
                      }
                      if (distance_to_leftmost > 88.0 && distance_to_leftmost < 136.0) {
                          nudge_threshold = 3.0;
                      }
                      if ((distance_to_leftmost > 68.0 && distance_to_leftmost < 72.0) ||
                          (distance_to_leftmost > 74.0 && distance_to_leftmost < 77.0) ||
                          (distance_to_leftmost > 60.0 && distance_to_leftmost < 61.0)) {
                          nudge_threshold = 2.0;
                      }
                  }
              }

              if (last_max_nudge_l < obs_l + nudge_threshold) {
                  sl_polygon->at(j).SetNudgeInfo(SLPolygon::RIGHT_NUDGE);
                  AINFO << "ID: " << sl_polygon->at(j).id() << " RIGHT_NUDGE";
              } else {
                  sl_polygon->at(j).SetNudgeInfo(SLPolygon::LEFT_NUDGE);
                  AINFO << "ID: " << sl_polygon->at(j).id() << " LEFT_NUDGE";
              }
            }
          } else {
            sl_polygon->at(j).SetNudgeInfo(SLPolygon::RIGHT_NUDGE);
      ...
      if (sl_polygon->at(j).NudgeInfo() == SLPolygon::RIGHT_NUDGE) {
        ...
        // if (!sl_polygon->at(j).is_passable()[RIGHT_INDEX]) {
        //   AINFO << "障碍物 " << sl_polygon->at(j).id() << " 黑名单已清除,允许绕行";
        //   continue;
        // }
