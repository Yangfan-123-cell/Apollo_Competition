...
bool ParkAndGoScenario::IsTransferable(const Scenario* const other_scenario,
  ...
  const Obstacle* nearest_type5_obstacle = nullptr;
  double nearest_type5_distance = std::numeric_limits<double>::infinity();

  for (const auto* obstacle : obstacles) {
    auto type = obstacle->Perception().type();
    if (type == 5) {
      const auto& pos = obstacle->Perception().position();
      const double dx = pos.x() - adc_point.x();
      const double dy = pos.y() - adc_point.y();
      const double dist = std::hypot(dx, dy);
      if (dist < nearest_type5_distance) {
        nearest_type5_distance = dist;
        nearest_type5_obstacle = obstacle;
      }
    }
  }

  if (nearest_type5_obstacle != nullptr) {
    AINFO << "最近的type==5障碍物 id=" << nearest_type5_obstacle->Id()
          << " 距离=" << nearest_type5_distance;
  } else {
    AINFO << "未找到type==5的障碍物";
  }
  
  // 如果最近的type==5障碍物距离小于7米，障碍物数量大于30且车辆静止，则切换为park_and_go
  if (is_vehicle_static && std::isfinite(nearest_type5_distance) &&
      nearest_type5_distance < 7.0 && obstacles.size() > 30) {
    AINFO << "满足近距离type==5障碍物且数量多且车辆静止，设置park_and_go=true";
    park_and_go = true;
  }
