...
hdmap::Lane::LaneTurn ReferenceLineInfo::GetPathTurnType(const double s) const {
  ...
  // 定义要排除的特殊车道 ID
  std::vector<std::string> excluded_lane_ids = {"Lane_709", "Lane_1731", "Lane_1669"};

  // 遍历当前参考线的所有车道段，检查是否命中特殊 ID
  for (const auto& seg : Lanes()) {
    const std::string& current_lane_id = seg.lane->id().id();
    for (const auto& excluded_id : excluded_lane_ids) {
      if (current_lane_id == excluded_id) {
        AINFO << "检测到排除车道ID: " << current_lane_id 
              << "，进入红绿灯无保护右转场景";
        return hdmap::Lane::RIGHT_TURN; 
      }
    }
  }
  ...
  for (const auto& seg : Lanes()) {
    ...
    const std::string& lane_id = seg.lane->id().id();
    
    // 打印当前检查的lane信息
    AINFO << "检查车道ID: " << lane_id 
          << ", 转向类型: " << static_cast<int>(turn_type);
    
    if (turn_type == hdmap::Lane::LEFT_TURN ||
        turn_type == hdmap::Lane::RIGHT_TURN ||
        turn_type == hdmap::Lane::U_TURN) {
      AINFO << "找到转向车道 - 车道ID: " << lane_id 
            << ", 转向类型: " << (turn_type == hdmap::Lane::LEFT_TURN ? "左转" :
                                turn_type == hdmap::Lane::RIGHT_TURN ? "右转" : "掉头");
      return turn_type;
    }
  }
  
  AINFO << "在位置s: " << s << " 处未找到转向车道，返回NO_TURN";
