...
// 构建障碍物在ST图(Space-Time图)中的边界
// 参数:
//   reference_line: 参考线
//   adc_start_s: 自动驾驶车辆(ADC)的起始纵向位置
//   st_boundary: 输出的ST边界
bool Obstacle::BuildTrajectoryStBoundary(const ReferenceLine& reference_line,
  ...
  for (int i = 1; i < trajectory_points.size(); ++i) {
    ...
    // 注意: 当参考线不是直线时,此方法会有误差
    // 需要双重循环来覆盖所有边界情况
    
    // 粗略跳过距离上次SL边界太近的点,优化计算
    ...
    // 计算ST边界的纵向范围
    // low_s: 障碍物起始位置减去ADC半长
    // high_s: 障碍物结束位置加上ADC半长
    ...
    // 从low_s和high_s向中间逼近,寻找实际发生碰撞的边界
    while (low_s + st_boundary_delta_s < high_s && !(has_low && has_high)) {
      if (!has_low) {
        // 检查在low_s位置,ADC包围盒是否与障碍物移动包围盒重叠
        ...
        has_low = object_moving_box.HasOverlap(
            {low_ref, low_ref.heading(), adc_length,
             // 注意: 这里的adc_width增加了nonstatic_obstacle_nudge_l_buffer
             // 这是一个横向安全缓冲区,用于给动态障碍物预留额外的横向空间
             adc_width + FLAGS_nonstatic_obstacle_nudge_l_buffer});
        low_s += st_boundary_delta_s;
      }
      if (!has_high) {
        // 检查在high_s位置,ADC包围盒是否与障碍物移动包围盒重叠
